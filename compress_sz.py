...
def compress(src, dst, truncpos=0, truncvel=0,):
    ...
    compression_opts = dict(
        Coordinates=dict(
            # TODO: performance? performance vs chunksize? SZ vs SZ3?
            # Open GH issue on SZ3 for performance/parallelization?
            hdf5=dict(
                chunks = (1<<26,3),
                # **hdf5plugin.SZ(absolute=1e6/1024/40/10),
                **hdf5plugin.SZ3(absolute=1e6/1024/40/10),
            ),
            truncbits=truncpos,
        ),
        # Velocities=dict(
        #     hdf5=dict(
        #         chunks = (1<<23,3),
        #         **hdf5plugin.SZ(pointwise_relative=0.0005)
        #         # **hdf5plugin.SZ(absolute=1.)
        #     ),
        #     truncbits=truncvel,
        # ),
        # ParticleIDs=dict(
